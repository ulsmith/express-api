...
export default class Request {
    constructor(globals, type, data) {
        ...
        if (types.indexOf(type) < 0)
            throw Error('Type does not exist, please add a type of request [' + types.join(', ') + ']');
        ...
        if (data.Records || data.rmqMessagesByQueue) {
            ...
            if (data.Records) {
                for (const record of data.Records)
                    this.requests.push(new Request(this.globals, this.type, record));
                return;
            }
            ...
            if (data.rmqMessagesByQueue) {
                ...
                for (const q in data.rmqMessagesByQueue) {
                    ...
                    for (let i = 0; i < data.rmqMessagesByQueue[q].length; i++) {
                        ...
                        this.requests.push(new Request(this.globals, this.type, {
    ...
    _awsRoute(data) {
        ...
        for (const key in data.headers)
            headers[DataTools.normalizeHeader(key)] = data.headers[key];
        ...
        if (process.__EAPI_PATH_UNSHIFT)
            rpath = rpath.replace(process.__EAPI_PATH_UNSHIFT, '');
        if (process.__EAPI_PATH_SHIFT)
            rpath = (process.__EAPI_PATH_SHIFT) + rpath;
    ...
    // @ts-ignore - Method is called dynamically via bracket notation in constructor
    _azureParse(data) {
        if (!data.req || !data.req.method || !data.req.url)
            throw Error('Azure integration only currently supports requests from http triggers');
        this.source = 'route';
    ...
    _socketRoute(data) {
        ...
        // normalize headers
        const headers = {};
        for (const key in data.socket.handshake.headers)
            headers[DataTools.normalizeHeader(key)] = data.socket.handshake.headers[key];
        // normalized request object
